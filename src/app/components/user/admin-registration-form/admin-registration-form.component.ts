...
import { AdminValidationService } from 'src/app/services/user/admin-validation.service';
...
export class AdminRegistrationFormComponent implements OnInit {
  ...
  private controlNames = ['firstname', 'lastname', 'dni', 'age', 'photo01', 'username', 'passwordGroup', 'passwordGroup.password', 'passwordGroup.confirm'];
  ...
  constructor(
    ...
    private pv: AdminValidationService,
  ...
  submit() {
    ...
    if (this.form.status === "VALID" && this.isValidCaptcha) {
      ...
      this.ps.registerAdmin(profile, this.pictures).then(uid => {
